...
import {formatDateModalEdit} from '@/utils';
...
export default function ModalEditProfile({
    ...
    const fetchAssets = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_BASE_URL}/select/company-asset`
        );
  ...
  useEffect(() => {
    ...
    const fetchIndustryClassification = async () => {
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_BASE_URL}/select/industry-classification`
        );
        const data = await response.json();
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BASE_URL}/company/update-company`,
        {
          method: 'POST',
          headers: {
    ...
    <div className="modal_wrapper pic">
      ...
      <div className="modal_box">
        <div className="steps_box">
          <div className={`step_number ${step >= 1 ? 'active' : ''}`}>
            Company Data
          </div>
          <div className={`step_number ${step >= 2 ? 'active' : ''}`}>
            About Company
          </div>
        </div>
        {step === 1 && (
          <div className="step">
            ...
            <div className="form_box">
              ...
              <input
                ...
                value={formatDateModalEdit(formData.ipoAdmissionDate)}
                onChange={handleChange}
                required
              />
            </div>
        ...
        <div className="button_wrapper">
          {step > 1 && (
            <button className="grey_btn" onClick={handleBack}>
              Back
            </button>
          )}
          {step < 2 && (
            <button className="green_btn" onClick={handleNext}>
              Next
            </button>
          )}
          {step === 2 && (
            <button className="green_btn" onClick={handleSubmit}>
              Submit
            </button>
          )}
        </div>
